...
// Only used to compute fourth root with known exponent
void
fp_pow_vartime(fp_t *out, const fp_t *x, const digit_t *exp, const int size, const int stop)
{
    fp_t acc;
    digit_t bit;
...
void
fp_ftrt(fp_t *out, const fp_t *x){
    // Exponentation by (p+1)/8 = c*2^{f-3}
    fp_t y;
    digit_t c;

    fp_sqr(&y,x);
    for(uint64_t i=1; i<TORSION_EVEN_POWER-3; i++){
        fp_sqr(&y,&y);
    }

    ibz_to_digits(&c, &TORSION_ODD);
    fp_pow_vartime(out, &y, &c, 1, 8);
}
